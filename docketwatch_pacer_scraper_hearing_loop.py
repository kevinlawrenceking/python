...
def main_loop():
    ...
    while True:
        try:
            main()
            print("Run complete. Sleeping 10 minutes before next run...")
            time.sleep(600)  # 10 minutes between runs; adjust as needed
            ...
            print(f"Exception in main loop: {ex}")
            if os.path.exists(LOCK_FILE):
                os.remove(LOCK_FILE)
            print("Sleeping 1 minute before retry...")
            time.sleep(60)  # Pause before retry
