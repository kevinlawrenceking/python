...
if len(sys.argv) > 1:
    court_url = sys.argv[1]
else:
    print("Usage: python insert_court_from_url.py <court_url>")
    sys.exit(1)
...
# Step 1: Ask GPT for court details
prompt = f"""
...
# Step 2: Lookup or Insert County
cursor.execute("SELECT id FROM docketwatch.dbo.counties WHERE county_name = ?", city)
row = cursor.fetchone()
...
if row:
    ...
    code = city[:5].upper()
    cursor.execute("""
        INSERT INTO docketwatch.dbo.counties (county_name, code, state_code)
        OUTPUT INSERTED.id
        VALUES (?, ?, ?)
    """, (city, code, state))
