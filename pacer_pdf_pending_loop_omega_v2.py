...
for row in case_events:
    ...
    # STEP 2: Run PDF download script - USING ORIGINAL extract_pacer_pdf_file.py
    print(f"[STEP 2] Running extract_pacer_pdf_file.py for case_event: {case_id}")
    try:
        subprocess.run([
            "python",
            "u:\\docketwatch\\python\\extract_pacer_pdf_file.py",
            str(case_id)
        ], check=True)
