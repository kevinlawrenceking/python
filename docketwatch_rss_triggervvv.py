...
def process_new_event(fk_case: int, event_no: int, court_code: str, cursor_arg=None, fk_task_run_arg=None):
    ...
    if case_event_id:
        ...
        if planned_docs:
            # For PACER documents, use the lightweight enhanced PDF downloader
            log_message(local_cursor, local_task_run, "INFO", f"Triggering lightweight enhanced PACER PDF download for case_event_id={case_event_id}", fk_case=fk_case)
            
            # Try lightweight enhanced downloader first (handles redisplay errors without Selenium issues)
            enhanced_cmd = ["python", r"u:\docketwatch\python\lightweight_enhanced_downloader.py", str(case_event_id)]
            pdf_result = run_subprocess(enhanced_cmd, "Lightweight Enhanced PACER PDF download", fk_case=fk_case)
            
            if pdf_result:
                log_message(local_cursor, local_task_run, "INFO", f"Lightweight enhanced PACER PDF download completed successfully", fk_case=fk_case)
            else:
                log_message(local_cursor, local_task_run, "WARNING", f"Lightweight enhanced PDF download failed, trying standard downloader as fallback", fk_case=fk_case)
