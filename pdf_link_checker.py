...
def get_db_connection():
    ...
    try:
        ...
        # Try to notify but don't fail if email fails
        try:
            error_notifier.log_database_error(error_msg)
        except:
            logger.error("Email notification failed, but continuing...")
        raise
...
def main():
    ...
    logger.info("=== PDF Link Checker Started ===")
    ...
    try:
        ...
        for i, row in enumerate(rows, 1):
            ...
            logger.info(f"Checking URL: {http_url}")
            ...
            if is_found:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error("Email notification failed for database error, but continuing...")
                    error_count += 1
                    
            else:
                # Log why PDF was not found, but don't update database (keep isfound = 0)
                not_found_count += 1
                logger.warning(f"✗ PDF not found: Case {fk_case}, Doc {doc_id} - {error_msg or f'HTTP {status_code}'}")
        ...
        print(f"PDF Link Checker completed successfully!")
        print(f"Total: {total_documents}, Found: {found_count}, Not Found: {not_found_count}, Errors: {error_count}")
        ...
        try:
            ...
            logger.error("Email notification failed for critical error, but continuing...")
