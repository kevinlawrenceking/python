...
def send_summary_email(doc_uid):
    ...
    (doc_uid, doc_id, fk_case, file_size, date_downloaded, summary_ai_html, 
     ocr_text, pdf_url, case_name, case_number, event_no, 
    ...
    docketwatch_link = f"http://docketwatch/court/case_details.cfm?id={fk_case}#docket"

    subject = f"DocketWatch PDF Summary: {case_name} Docket No. {event_no} [{event_desc}]"
    body = f"""
    <html><body>
        <p><strong><a href=\"{docketwatch_link}\">DocketWatch</a></strong> | 
        <strong>PACER PDF:</strong> <a href=\"{pdf_url}\">{pdf_url}</a></p>
        <h2>{case_name}</h2>
        ...
        <strong>PDF:</strong> <a href=\"{virtual_path}\">{doc_id}.pdf</a><br>
        <hr>
        <h3>AI Summary</h3>
        {summary_ai_html}
        <hr>
        <h3>OCR Text</h3>
        <pre style='white-space: pre-wrap; font-family: monospace;'>{ocr_text}</pre>
    </body></html>
    """
