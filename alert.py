...
# ** Configure Logging **
LOG_FILE = r"\\10.146.176.84\general\docketwatch\python\logs\docketwatch_surrogate_scraper.log"
logging.basicConfig(filename=LOG_FILE, level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
logging.info("=== Surrogate's Court File Search Started ===")

# ** Prevent Multiple Script Instances **
LOCK_FILE = r"\\10.146.176.84\general\docketwatch\python\docketwatch_surrogate_scraper.lock"
...
try:
    ...
    # ** Select Court - New York County Surrogate's Court **
    try:
        court_value = "lvbt9e9D8n4m0vSXdSUing=="  # New York County Surrogate's Court
        ...
        logging.info("Successfully selected New York County Surrogate's Court.")
    ...
    # **UPDATE alert_last_checked timestamp (ALWAYS RUNS)**
    update_last_checked_query = """
        UPDATE docketwatch.dbo.utilities 
        SET alert_last_checked = GETDATE()
        WHERE id = 1
    """
    cursor.execute(update_last_checked_query)
    conn.commit()
    logging.info("Updated alert_last_checked timestamp (Checked regardless of result).")
    ...
    driver.quit()
    os.remove(LOCK_FILE)
    logging.info("Surrogate's Court File Search Completed Successfully!")
