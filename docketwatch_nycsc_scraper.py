...
log_message(cursor, fk_task_run, "INFO", "=== New York County Case Scraper Started ===")
...
# === SETUP CHROME WITH UC ===
chrome_options = uc.ChromeOptions()
chrome_options.add_argument("--no-sandbox")
chrome_options.add_argument("--disable-dev-shm-usage")
chrome_options.add_argument("--user-data-dir=C:/Temp/ChromeProfile")

driver = uc.Chrome(version_main=137, options=chrome_options)
...
if "Just a moment" in driver.page_source or "cf_chl_opt" in driver.page_source:
    log_message(cursor, fk_task_run, "INFO", "Waiting for Cloudflare interstitial...")
    WebDriverWait(driver, 60).until_not(lambda d: "Just a moment" in d.page_source or "cf_chl_opt" in d.page_source)
    log_message(cursor, fk_task_run, "INFO", "Cloudflare challenge passed")

# === LOGIN ===
cursor.execute("SELECT nyc_user, nyc_pwd FROM docketwatch.dbo.utilities WHERE id=1")
credentials = cursor.fetchone()
if not credentials:
    log_message(cursor, fk_task_run, "ERROR", "No credentials found. Exiting...")
    sys.exit()
...
try:
    ...
    log_message(cursor, fk_task_run, "ERROR", f"Login error: {login_error}")
    driver.quit()
    sys.exit()
...
for court in courts:
    ...
    log_message(cursor, fk_task_run, "INFO", f"=== Scraper Started for {court_name} ({court_code}) ===")
            ...
            try:
                ...
                for row in case_rows:
                    try:
                        ...
                        if "*****<br>*****" in row_html:
                            log_message(cursor, fk_task_run, "INFO", f"Restricted case skipped for {court_code}")
                            continue
                        ...
                        cursor.execute(
                            "SELECT id FROM docketwatch.dbo.cases WHERE case_name = ? AND case_number = ?",
                        ...
                        if not exists_row:
                            ...
                            cursor.execute(insert_query, (
                                ...
                                case_type, case_status, "", fk_task_run))
                            case_id = cursor.fetchone()[0]
                            conn.commit()
                            log_id = log_message(cursor, fk_task_run, "INFO", f"Inserted and processed case: {case_name} for {court_code}", fk_case=case_id)
                            process_case(case_id, case_number, case_name, court_code, county_code)
                        else:
                            case_id = exists_row[0]
                            log_message(cursor, fk_task_run, "INFO", f"Case already exists: {case_name} ({case_number}) for {court_code}", fk_case=case_id)

                    except Exception as row_error:
                        log_message(cursor, fk_task_run, "ERROR", f"Error processing row for {court_code}: {str(row_error)}")
                        continue
                ...
                try:
                    ...
                    log_message(cursor, fk_task_run, "INFO", f"No more pages found for {court_code}. Updating timestamp.")
                    cursor.execute("UPDATE docketwatch.dbo.courts SET last_scraped = GETDATE() WHERE court_code = ?", (court_code,))
                    conn.commit()
                    break

            except Exception as e:
                log_message(cursor, fk_task_run, "ERROR", f"Error scraping {court_code}: {str(e)}")
                break

    except Exception as fatal_error:
        log_message(cursor, fk_task_run, "ERROR", f"Fatal error for court {court_code}: {str(fatal_error)}")

log_message(cursor, fk_task_run, "INFO", "=== NYC Court Scraper Complete ===")
driver.quit()
